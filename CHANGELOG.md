...
- **Phase 2 Docker RC qualification 邊界固化**：`review-verdict` 僅保留為 Phase 2a legacy fallback，R9 將其列為 deny-only asset，權威寫入路徑固定為 `review-verdict-spool`；probe 使用隔離的 root-owned protected legacy parent，避免 builder worktree default ACL 汙染測試；rootless Docker fixture 的 restore 也改由合成 owner 還原，不放寬 production ACL。
