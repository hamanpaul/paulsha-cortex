...
- **Manager接管workflow單一寫入與嚴格phase spine**：Deck emit會同步fsync持久化persona-preserving workflow manifest；control queue新增`workflow-action`，只有Manager能建立或推進`WorkflowRun`，並在Define/Plan production path透過safe launcher runtime實跑completeness、live model probe與異質brainstorm。phase固定為`claim→define→plan→build→verify→review→ship`且強制persona spine；每次推進會把executor/model/domain/output/gate result寫回step。gate evidence必須實際讀取並驗schema/content hash/phase/domain/candidate，只有被completeness觸發的run要求brainstorm，進ship另要求已驗證candidate、ForeignReview與current exact-HEAD Copilot皆passed；registry在atomic replace後directory fsync失敗時會以fsync backup復原檔案並重載記憶體。
