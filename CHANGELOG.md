...
- **Manager接管workflow單一寫入與嚴格phase spine**：Deck emit會同步fsync持久化persona-preserving workflow manifest；control queue新增`workflow-action`，只有Manager能建立或推進`WorkflowRun`，並在Define/Plan production path實跑completeness與異質brainstorm。phase固定為`claim→define→plan→build→verify→review→ship`，gate evidence改為kind/ref typed且locator互異，passed要求brainstorm與ForeignReview、ship再要求Copilot；registry在atomic replace後directory fsync失敗時會以fsync backup復原檔案並重載記憶體。
