...
`retire-delivered` 只會移除同時 pinned 在該 run planning authority 與目前 mapped Todo、且已不存在的 path link；存在、symlink 或未經這兩者確認的連結會保留。
...
verify／review steps 全部通過後，若 operator 才提交後續阻斷發現，仍可對尚未完成的 run 使用 exact-candidate `retry-build`；需附 `--reason`，Manager 會以既有 immutable operator-adjudication evidence 記錄裁決，再重開 builder 並重新驗證新候選。候選不符或 run 已完成時會拒絕。
...
quota 不會在同一個 identity 上重試；Builder roster 有其他合格 identity 時會改派下一位，沒有替代 identity 時則停止。這項改動不調整 rate-limit、transient 或其他 provider outcome 的 retry 政策。
