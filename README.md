...
- canonical `.project-policy.yml` 或 legacy `.paul-project.yml` 一旦存在即必須明確宣告 `tier`；缺失或非法值會在 builder dispatch 前 fail-closed。未提供任一 manifest 的 repo 才沿用 `shareable` 預設；允許值為 `shareable`、`work`、`personal`。
