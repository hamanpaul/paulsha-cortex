...
Manager以canonical brainstorm evidence中的scope與artifact ref/kind/hash，把新發布planning artifacts與既有authority做CAS式合併，並另存不可變的brainstorm發證source revision；run目前source revision可隨PR refresh更新但不得改寫發證revision。Legacy active run也只可由相同evidence補齊，brainstorm-required run缺evidence或evidence/file drift一律停止。之後launch前解析declared inputs，accepted planning artifact只接受WorkflowRun authority中的ref/hash。獨立builder worktree缺檔時由Manager原子seed；Job、prompt、terminal evidence保存相同input snapshot，terminalize再次驗hash。Prompt同時保存card skill/action/commit/test policy與exact terminal schema。Legacy pending card可繼承同phase inputs，但passed card不回填新gate；dispatch exception必須恢復`needs_human`。
