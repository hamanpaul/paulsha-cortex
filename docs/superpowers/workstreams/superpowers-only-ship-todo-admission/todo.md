...
- 此 parent packet 目前是 draft、預估 10／Red。不得標記 accepted、intake、dispatch 或視為實作授權。Child A #1054、Child B #1055 已是 issue-backed；兩票分別 review／重算 sizing，且 #1055 hard-depends on #1054。
...
- [ ] **T5 recovery／#983 exact Candidate protocol（R4；child #1055）**：在 #1055 凍結同 run CAS 或新 run adoption 的唯一合法流程；hard-depends on #1054 admission/diagnostic contract。比對 run id、claim key、old/new `source_revisions`／authority digest、exact candidate／verified head、job bindings、PR number/head 與 delivery binding。逐類宣告舊 verify／review／delivery evidence 保留或失效，必要時從同一 exact candidate 重驗。任何 conflict／unknown 不得改寫 registry、journal 或 source revision。
...
1. **Admission child #1054**（[live issue](https://github.com/hamanpaul/paulsha-cortex/issues/1054)）：title `fix(work): Superpowers-only work 在 Builder dispatch 前要求唯一 Todo source`；owner 為 Manager admission/diagnostic；WorkAuthority／Monitor snapshot 是唯讀 source of truth；無 Child B 前置依賴。只接 #1054 的五項 acceptance criteria、R1–R3、T1–T4 及 #1051 AC1–2／AC4 前五種 source/build admission case。
2. **Recovery child #1055**（[live issue](https://github.com/hamanpaul/paulsha-cortex/issues/1055)）：title `fix(recovery): Todo authority 加入後安全恢復已有 Candidate 與 PR 的 run`；owner 為 Manager recovery，與 claim/evidence/delivery owners 協作，以 GitHub PR facts 作遠端事實。Issue #1055 明示 Blocked by #1054；hard dependency 是 #1054 落地 unique-Todo admission、可執行 diagnostic 與 fresh Monitor snapshot contract。只接 #1055 的五項 acceptance criteria、R4–R5、T5–T6 及 #1051 AC3／AC4 existing-candidate/PR case。

兩張 child issue 已建立且維持 open；本 packet 仍是 draft，不代表兩票已 accepted／intake。各 child 開工前須依自己的 issue scope 建立／review accepted planning packet 並重算 sizing；#1055 在 #1054 落地前不得開工。此拆分不以空泛「先研究」取代原 issue AC；#1055 必須交付明確可執行恢復流程，否則 #1051 不可宣稱完成。
