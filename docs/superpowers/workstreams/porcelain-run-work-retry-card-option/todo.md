...
5. Existing `--payload` card-only workaround remains accepted; unrelated extra evidence refs remain forwarded. Explicit `--card` cannot be shadowed by a conflicting payload card.
...
- [ ] **T3 docs / policy surfaces**: README run-work examples gain retry-card command with exact run id/card and optional builder pair; clarify payload is for additional manager-side evidence refs. Add implementation changelog fragment and `[Unreleased]` line; keep VERSION unchanged.
...
Complete #1030 when the explicit selector is discoverable and correctly forwarded only for retry-card, misuse and invalid input fail before enqueue, payload-only compatibility and extra refs remain deterministic, README/help/tests are updated, and repository delivery checks pass. Manager retry policy and downstream job outcomes remain governed by their existing evidence.
