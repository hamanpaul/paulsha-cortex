...
- [ ] **T2 source／ship 入口與 binding**：`_ship_action` 入口 gate 改為 `len(mapped_prs) != 1 or
...
- [ ] **T3 source／GitHub facts 與 gates**：`GitHubDeliveryClient._openspec_facts` 接受 `change=None`，
...
- [ ] **T4 source／delivery 與 completion**：`delivery._validate_work_authority` 把
...
- [ ] **T5 source／review-attest（C）**：`_review_attest_action` 在 `mapped_openspec == ()` 時以
...
- [ ] **T6 tests／回歸**：`tests/test_work_actions.py`、`tests/test_delivery_orchestrator.py`、
...
- [ ] **T7 documentation**：`docs/unified-work-lifecycle.md` 的 ship 段（「ship transition 現固定分成
