...
- [ ] **T6 compatibility, docs and changelog (R8; policy surfaces R-09/R-16/R-19; AC 7)**: Preserve JSON roots/schema_version, public method return values, single-writer semantics, atomic fsync/rollback and v1 backup behavior. Keep the existing rollback and migration regressions green. Update README.md’s shared coordinator-root contract and docs/unified-work-lifecycle.md’s durable registry description to explain stale-revision rejection and explicit request error. Add the eventual feature’s changelog.d fragment and CHANGELOG.md [Unreleased] entry per repo policy. Do not add a CLI.
