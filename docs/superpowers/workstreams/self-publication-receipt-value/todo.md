...
- Child A of #978/#963/#847; live issue is #992. Accepted contract: [spec](../../specs/self-publication-receipt-value-spec.md) and [design](../../specs/self-publication-receipt-value-design.md).
...
- [ ] T1 — typed envelope: add frozen receipt and producer-specific accepted-input/evidence/object/transaction variants with exact keys/types; require planning `transaction.kind` to equal the corresponding exact intent-core schema; recompute receipt/event/publication/intent digests; reject cross-bindings. Document that the typed parser does not prove authority-digest provenance; producer owners must validate the current trusted run/claim binding.
...
- [ ] T5 — collisions and identities: test same event/different publication allowed; exact pair replay stable; conflicting payload, duplicate publication across events, same output ref across different pair, invalid receipt ID, source_revision/commit-SHA/artifact-SHA type confusion rejected; confirm the plan logical workflow-run identity is derived and not confused with the serialized snapshot ref.
