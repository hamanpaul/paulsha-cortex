...
**OpenSpec archive:** `openspec/changes/archive/2026-07-12-dispatch-discipline-improve/`；change 已完成並正式 archive，`tasks.md`保留apply進度，本文件是exact files、RED/PASS commands與commit boundaries的historical execution plan。

以下歷史commit範例中的`$OPENSPEC_TASKS`代表執行當時active change的`tasks.md`；目前權威完成紀錄為上述archive內的`tasks.md`，不可把archive path誤當成實作期間的mutable tracker。
...
- 歷史執行時，每個Task在focused/full gate通過後都必須更新apply progress；完成紀錄現存於`openspec/changes/archive/2026-07-12-dispatch-discipline-improve/tasks.md`。
...
```bash
git add paulsha_cortex/coordinator paulsha_cortex/scripts/reap-codex-brokers.sh tests CHANGELOG.md "$OPENSPEC_TASKS"
...
```bash
git add paulsha_cortex/coordinator paulsha_cortex/control tests CHANGELOG.md "$OPENSPEC_TASKS"
...
```bash
git add paulsha_cortex/coordinator tests CHANGELOG.md "$OPENSPEC_TASKS"
...
```bash
git add tests/test_coordinator_dispatch_discipline_e2e.py README.md CHANGELOG.md docs/superpowers/specs "$OPENSPEC_TASKS"
git commit -m "test(coordinator): cover dispatch discipline canary"
```

## Implementation handoff

1. 歷史執行時先把本spec、plan與當時的active OpenSpec change納入feature integration branch，再從該branch建`wt/dispatch-discipline/reaper`執行Task 1；完成後的權威產物現位於`openspec/changes/archive/2026-07-12-dispatch-discipline-improve/`。
