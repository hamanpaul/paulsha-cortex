...
receipt_id is H("cortex-self-publication-receipt/v1", envelope with receipt_id omitted). `pre_publication_authority_sha256` is the lowercase 64-hex WorkAuthority canonical digest bound to this exact run/claim at trusted claim or refresh time; it is not an artifact byte hash. When a later Manager producer no longer has the `WorkAuthority` object, it may reuse `WorkflowRun.source_revision` only from the current persisted Manager-owned run row, never from caller arguments or a reconstructed caller dictionary, and only when the producer verifies the exact `(repo, work_id, run_id, claim_key)` row plus `claim_key_for_authority_digest(repo=repo, work_id=work_id, authority_digest=source_revision) == claim_key`. The producer must capture the full-registry raw-byte revision before its first sidecar/workspace mutation, preserve the same source-revision/claim-key binding through publication, and submit that held revision to Child B. Any missing/untrusted row proof, non-lowercase-64-hex or mismatched digest/key, authority restart/refresh/reclaim after capture, changed binding, or stale append revision fails closed with no receipt. The value parser validates shape and digests, but cannot establish this origin proof; no extra envelope key is added.
...
- [ ] Exact v1 envelope and all three exact producer unions have executable canonical golden vectors, including planning transaction-kind-to-intent-schema mapping, derived plan logical identity versus serialized snapshot ref, IDs, digests, marker grammar and rejected extra/missing fields.
...
- [ ] The value schema documents that `pre_publication_authority_sha256` is provenance-sensitive: producers must supply the current trusted run/claim-bound WorkAuthority digest; a bare `source_revision` value or syntactically valid digest is insufficient.
...
WorkflowStep has no step_id/card_id fields. Producer identity is the exact unique pair phase=plan and WorkflowStep.card value plus the non-negative integer `run.attempts["plan"]` snapshot. If zero/multiple plan steps match that card or phase attempt is absent/invalid, do not mint; never substitute tuple position or list row index. The `workflow-run://<encoded-run-id>/phases/plan/cards/<encoded-card>/attempts/<phase-attempt>/plan-acceptance` string is a derived logical identity only and is not serialized as a ref. The serialized `acceptance_evidence.ref` is the retained `planning-input-snapshots/<encoded-run-id>/plan/<phase-attempt>/<encoded-card>.json` sidecar below; the logical identity is verified from the exact run/phase/card/attempt facts.
