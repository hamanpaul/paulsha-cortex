...
新增測試覆蓋 `ReviewLoop` 的提交／觀測分離與 900 秒邊界，並以既有 `_ship_action` fake-GitHub harness 走真實 caller 路徑確認 #1020 時序。沿用既有 final-gate matrix 和 #948 adoption tests 做 regression evidence。若整合測試揭露 caller 必須改 production code，先擴大 production module mapping、domain/state 宣告及 sizing，再實作。

### D5 Sizing projection
