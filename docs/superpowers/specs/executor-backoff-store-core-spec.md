...
1. **R1 Three-state bounded observation**：獨立 `<coordinator_root>/executor-backoff.json`、schema `executor-backoff/v1`，每次 store query 重新讀磁碟，遵守 design D2.1 的固定容量 profile。先作有限讀取／結構計數，再 decode；不可先無界 read/loads/dumps 才檢查長度。只有真缺檔可回 `missing`；可解析且 schema／資料／durability 驗證通過才 `valid`；read error、corrupt、未知 schema／policy、超限或無法確定持久狀態均 `unknown`。missing 只是無本機已知 backoff，valid 無 active entry 也不是 quota-positive 證據。`unknown` **可在可恢復時**附同 scope persisted last-known floor 供診斷與保守保護界使用，但它不是獨立再驗證的 admission 證據，不能把其缺少的 identity 當可用；不得以 corrupt→empty 自癒覆寫。
...
- 唯一新增 **runtime logic** 仍為 `paulsha_cortex/coordinator/executor_backoff.py`；同模組容納 types、schema validator、pure fold、store 和對帳 seam。`paulsha_cortex/coordinator/__init__.py` 只允許 package-level compatibility export，不新增第二份 runtime 行為。可以唯讀重用 `backoff.tick_backoff_seconds`，不修改其他 production logic。
- 本 worktree 現已交付 child A 的產品工作、tests、changelog 與 task tracking；registry/terminal provenance、production caller inventory sourcing，以及 workflow/slice/request/daemon lane 接線仍由後續正式工作執行。
