...
- 每個經 production Manager needs_human wrapper 的 probe failure 都必須寫入 durable delivery evidence，並將 evidence ref/hash 隨 stop 結果提供。使用既有 `work_bridge._write_json_evidence` 的 content-addressed JSON envelope；測試從該次結果提供的 ref 重新讀檔，驗 envelope hash 並逐欄比對 structured payload。payload 至少保留 Candidate、`stage`、`returncode`、`error_kind`、`main_head`；不得以 transient typed object 或 exception/detail prose 代替 persisted evidence。第一次 fetch failure 的 read-back 須證明 `stage=fetch`、`returncode` 等於該子程序實際非零退出碼、`main_head=null`；成功取得合法 M 後再發生的 failure，其 read-back 須保留同一精確 M 及原 stage/returncode/error_kind；timeout 須是 `returncode=null`。這是 delivery evidence read-back，不取代 child 04 的 `needs_human_reason.context.main_sync` durable read-back。
- 整合 regression 必須證明 `fetch` failure 經 Manager 形成 `main-sync-unavailable` needs_human stop 後，先從該 stop 所指向的 durable delivery evidence ref 讀回並斷言第一次 `fetch` 的實際 returncode 與 `main_head=null`；再修復 bare-origin 的環境條件，從正式 operator workflow `resume` path 進入 ship validator、重新執行 bounded probe 並觀察修復後的新 M。此 child 驗證 stop/re-entry，不取代 child 04 的 nested-context read-back。測試必須確認不是 `work_actions._claim_action` 對非-define run 呼叫 `workflow_starter` 的 no-op 就算通過。
