...
| 4 | `cg` × builder | cg 是 zero-tool（`build_cg_argv` docstring，`launcher.py:703-754`：wrapper 自帶 `--available-tools=__none__`＋`--disable-builtin-mcps`＋throwaway HOME，不能跑 tool／寫檔／commit）。`manager._workflow_identity_candidates_for_persona` 會在 builder 候選解析時排除 `cg`；建構期仍有三重 fail-closed：`allow_unsafe` raise（`launcher.py:735-736`、`813-814`）、`commit_required` raise（`launcher.py:737-738`）、builder 語境（`read_only`／`review_only` 皆 False）raise（`launcher.py:739-740`、`820-821`）。`#442` 已確認「補 cg builder」這條路走不通。 |
