...
`verify-attest`（#882）是 verify 停在 `needs_human` 時的 operator 出口，必須提供 `--expected-candidate`、`--actor` 與 `--payload`；payload 僅接受 `full_suite_command` 及 `result_summary: {passed, failed}`，且 `failed` 必須為 0。這是 operator 提交的測試命令與結果聲明，Manager 不會代跑該命令。Manager 將 repo/work/run/authority/candidate、命令、結果摘要與 actor 寫成 immutable evidence，再以 registry 內的 exact Candidate CAS 將 verify steps 標 passed、`verified_head` 綁定同一 candidate 並推進到 review；已完成的 build 與無 active job 是前置條件。這只替代被 operator 覆核的 verify gate，後續 foreign review 與 ship/review gate 照常執行，`review-attest` 的 review-only 契約不變。
...
**`retry-card` 一併涵蓋 reviewer 卡（#569）。** 同一個 run 的 verify 階段之後重現了同型死結：verification job（agy，#568 的權限剖面缺陷）exit 0 但 log 完全沒有 JSON envelope，harvest 每次都撞 `workflow terminal log has no JSON evidence`。reviewer 卡當時沒有等價出口——`retry-verify` 是 slice-lane 時代的 **phase 級**重置，它清掉 `needs_human` 與 verify step 卻**不在同一個 action 內派新 job**（實測回應 `job: None`），run 因此四小時對 tick 隱形後 `needs_human` 原地回鍋。`retry-card` 的受理範圍因此放寬到 verify／review phase 的 reviewer 卡（`verification`／`code-review`／`adversarial-review`），CAS、卡名定錨、evidence immutable、facet 原子性、舊 job 不動等硬約束逐條沿用：`--card` 必須正是當前 phase 內最早一張非 passed 的卡且 persona 與 phase 相符；已對**現在這個 candidate** 綁定 `workflow_evidence` 的卡拒絕重派（上一代 candidate 的歷史 evidence 不參與判斷，否則換過 candidate 之後會再造同一個死結）；新 job 的身分由 identity registry 在 dispatch 當下**重新解析**，不複製舊 job 的 executor／model（#568 的 reviewer fail-over 依賴這一點）；被取代 job 的 reviewer sandbox 在派新 job 前原子回收，回收時若發現 candidate checkout 已被改動則 fail closed。`retry-verify`／`retry-review` 的 CAS 與 admission 一字未改（`docs/superpowers/specs/fix-repair-commit-recovery-spec.md` R4）。兩者 reset 標記舊 exited reviewer job 為 failed 前，Manager 會先套用精準 terminal recovery 判準；符合者保留 `exited`，讓既有 recovery 判準繼續生效。#883 起，public `retry-verify` work-action 在 phase reset 後於同一 request 強制派出新的 verification job；符合 #577 精準復原條件的舊 job 仍保留 `exited`，派工未產生新 job 時沿既有補償恢復 `needs_human`。帶 reviewer identity override 的 `retry-card` 也會在 registry reset 前以相同 identity 判準驗證，因此失敗不留下 run override。
...
**verify／review malformed terminal 的 periodic 有界自癒（#578）。** job 正常以 exit 0 終止、尚無 `workflow_evidence`，但 terminal 無法解析或不符合該 phase 的外層 schema 時，periodic resume 會沿用既有 per-card schema retry 計數與上限自動重派；耗盡後轉為 `needs_human`。形狀合法的 `failed`／`needs_human` 仍走 #874 明示停止，不重派；有效 envelope 後續的 gate／authority 驗證錯誤也不納入此 retry。operator 的 `retry-card` 仍受 #555 per-card 熔斷限制。

**needs_human 的 next_actions 會反映目前實際可受理的出口（#546、#569）。** `ClaimCandidate` 帶入由 canonical job／owner-bound slice 判準導出的 recovery actions，claim/resume、Monitor 的 `cortex list`／`cortex work show` 與 `cortex status` attention 共用同一份投影。build／verify／review 的 `regenerate-gates`／`retry-card`、符合條件的 `retry-build`／`retry-review`，以及 owner slice 可受理時的 `recover-pre-candidate` 才會列出；只宣告會被受理的動作，證據讀取失敗或 owner 身分無法唯一確認時不宣告對應出口。對 `blocking-findings` 的 review run，符合各自前置條件時依序宣告 `retry-review`（接受裁決）／`retry-build`（駁回裁決），並附上可照抄的指令提示。`review-attest` 對 rejected review gate fail closed。

**request 完成 ≠ Job 啟動 ≠ phase 推進：派工結果的四類契約（#830、#833）。** `work start`／`intake`／`resume`／`retry-*` 與 periodic continuation 送到 Manager 的派工結果只有四種：**真 Job**（回 `job_id`，且該 Job 在 registry 中綁定到同一個 run；forged job_id 或綁到別的 run 一律拒絕）、**合法非 Job 決策**（producer 回 `{run_id, current_phase, reason}`，例如拆分深度逾限的 `decomposition-depth-exceeded`、`runtime-preflight-*`、`plan-outputs-missing`；不造 job_id、不算 dispatched、不啟動模型）、**確定性 transition**（沒有 Job 但 run 已持久化推進 phase）、**None**（沒派也沒推進，維持 not-dispatchable）。plan 完成且 Red 尚未達拆分深度上限時，Manager 保留 parent 的 plan phase 與 `needs_decomposition` facet，持久化並派出唯一的 `red-decomposition` planner Job；同一 run 重送會重用此 Job，不會繼續派原工作 build。拆分計畫通過既有 plan review gate 後，Manager 透過 planning transaction 僅發布該 child 的 `.cortex/work-items.yaml` 項目與 `docs/superpowers/workstreams/<child>/todo.md`；periodic resume 重送冪等。標準 intake 僅在 Monitor snapshot 確認該 child 的 WorkAuthority 後受理，snapshot 尚未更新時 parent 保留 `needs_decomposition` 並等待後續 tick。request 回應以 `dispatch.kind` 呈現這四類並附最新持久化的 run；真 Job 才另帶 `job_id`。合法決策不會因 adapter 例外被降成一般 `needs_human`／`resume-workflow-failed`，重送同一 start 也不會新增 run 或 Job；forced retry（`retry-build`／`retry-card`）的成功後置條件仍是「新 replacement Job」，遇到決策或 None 一樣走既有 fail-closed 補償並把 producer 的 reason 帶進診斷。
**slice terminal job 依目前 binding 收割（#481／#497）。** 對已有 slice row 的 terminal builder／reviewer，`complete_tick` 只處理仍分別綁定於 `builder_job_id`／`reviewer_job_id` 的 job；舊 attempt 僅保留稽核，不寫 evidence、不改 slice。現行綁定 job 的 manifest 已指向同一 job，且 slice 已反映該終局時，後續 tick 不重跑；`verification-runner-error` 已落成 `needs_human` 時也不重驗。若前次 state mutation 失敗、slice 尚未反映終局，仍允許同 job 修復重試。`recover-pre-candidate` 清除 binding 並持久化舊 job 的 supersession 後，registry 重載仍會略過該 job。registry 真正查無 slice row 時仍保留既有 `missing-slice-proof` 流程。
...
**升級與運維。** 派工 prompt 現在發的是 canonical envelope（`schema_version: 2`，多帶 `diagnostics` 與 `gate_evidence`）。不帶該版本的舊 payload 仍走相容讀取路徑，不會因版本差異被拒收。切換當下已在飛行、且沒有 gate ledger 的 build run，其 `passed` terminal 會 fail closed 並轉 `needs_human`——這是預期行為（沒有獨立證據就不放行），不是資料損毀：candidate 與 worktree 都還在，只是未被授權。verify phase 不要求 wrapper gate ledger，通過與否由 deterministic verification report 管線提供獨立證據。處理方式是對該 build run 重新派工該張 card（resume 會以新的 wrapper 重跑並產生 ledger），不需要 abandon 整個 work item；只有在 candidate 本身已被判定不可用時才需要 abandon 重跑。
