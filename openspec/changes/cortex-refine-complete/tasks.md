...
- [x] 2.7.2 #824 dependency 證據未滿前不登錄可 claim child／不開 auto label；解除後重新核對完整性、真 sizing、唯一 owner 與正式 freeze，不能依 custom frontmatter 自稱已鎖派工。
  ...
  - [x] repair：以實際 `agy` binary 執行 D4 五列 `--print-timeout` parser 負控制，將 merged stdout/stderr 與 exit code durable transcripts 落到 `docs/evidence/agy-print-timeout-only-logs/*.txt`，並在 local validation 記錄 exact argv、bytes 與 SHA256；不宣稱 archive／merge／issue closure／loaded-runtime 完成。
...
- [ ] 2.8.1 先依 #850 有界 store／immutable event-fold child 交付 component；C/D provenance、reconciliation 與所有 lane 接線仍由 #825 後續 child 承接。
  ...
  - [x] GREEN：新增 `paulsha_cortex/coordinator/executor_backoff.py` 與 package
    ...
    focused `tests/test_executor_backoff.py` 轉綠；schema v1／bounded capacity／
...
- [x] 4.1 完成 #826 failure 原始訊號→持久化→消費端分類，保留 runtime-contract 硬阻擋。
  - [x] RED：新增 `tests/test_outcome_taxonomy_signals_826.py` 與缺 handle completion 回歸，
        鎖定 copilot effort、exit 127 空日誌、dispatcher missing handle、autonomy／
        workflow launch exception 與 workflow reroute/contract consumer wiring；正式
        taxonomy、詞彙文件與 GREEN 修補仍待後續卡片。
  - [x] GREEN：補上 `effort_not_supported`／`executable_not_found`／`launch_failed`
