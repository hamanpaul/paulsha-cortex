...
- [ ] [GREEN] 以 quote-aware flow-sequence tokenizer 修正 `_yaml._parse_scalar`，並使聚焦與既有 consumer suite 通過。
- [ ] [DOC] 新增 changelog fragment，並同步 `CHANGELOG.md` 與 README 的 YAML／argv 引號說明。
- [ ] [CLI] 執行 `python3 -m paulsha_cortex.cli --help` 與 `python3 -m paulsha_cortex.cli deck --help`。
- [ ] [VERIFY] 執行完整驗證關卡並保存 candidate 與驗收證據。
