...
Manager執行quick policy及configured CI-parity preflight子行程時 MUST移除所有繼承的`PSC_*` runtime authority，並 MUST使用完成後刪除的disposable `HOME`與`XDG_CACHE_HOME`；必要的Python user-site與GitHub config等工具／認證root MAY顯式保留。Manager service MUST固定deterministic umask。Preflight內的測試 MUST NOT從process env或installed bootstrap重新取得production coordinator root、executor、repo或service identity。
...
- **THEN**兩個gate子行程均看不到繼承的`PSC_*` runtime authority，且HOME/cache指向同一個disposable root
- **AND**必要的非Cortex工具與認證root仍可供preflight使用，disposable root在gate結束後刪除
- **AND**service umask固定為0022，operator umask不得改變suite的預設檔案mode
