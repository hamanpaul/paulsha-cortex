...
所有planner invocation在temporary disposable checkout執行，CLI仍強制plan/read-only/sandbox；`finally`比較sandbox與operator checkout的tracked/untracked全內容hash，nonzero/exception亦不可跳過，operator污染會由baseline回復。Primary integration只回structured content；Manager依current work/change與compiled manifest output refs驗path，在staging驗完整組後以no-clobber hard-link transaction publish，任一失敗回滾本次全部產物。
