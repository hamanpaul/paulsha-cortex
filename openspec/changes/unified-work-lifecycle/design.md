...
所有planner invocation（包含manifest plan card）在temporary disposable checkout執行，Claude固定`plan`並停用tools、Codex固定`--sandbox read-only`、agy固定plan/sandbox；tree snapshot涵蓋檔案內容、empty dirs、directory symlinks與stable metadata，nonzero/exception亦不可跳過，operator污染會由baseline回復。Primary integration只回structured content；Manager依current work/change與compiled manifest output refs驗path。新檔使用原子no-clobber，既有TBD artifact只在current hash等於scan baseline時CAS replacement；artifact、immutable brainstorm evidence與registry phase update寫入durable intent journal，save fault rollback整組，restart依persisted brainstorm gate commit或rollback journal。
...
Builder與Foreign Reviewer沿用Candidate verification/review，但reviewer必須不同domain且在detached exact HEAD。Verify/review terminal payload必須列出符合manifest output glob且可由repo root重讀/hash的真實report；canonical coordinator evidence locator只屬gate ref，不算report output。Brainstorm peer evidence與ForeignReview evaluation保存不同step/gate refs，不可互換；Copilot也只屬ship gate。
