...
- [x] 補上 registry `build` capability 與 writable launcher 形狀的機械對應測試（預期 mode／sandbox／
      worktree 由 capability 導出）；commit-required Git metadata 與真實 launcher argv forwarding 均有
      回歸測試；default／unsafe／commit-required 無 worktree 的 builder 形態皆 fail-closed。
