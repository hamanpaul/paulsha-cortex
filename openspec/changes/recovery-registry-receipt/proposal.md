...
# Recovery registry disposition 與 atomic receipt 提案

## Why

Recovery 需要可持久重送、exact binding 與 ABA 世代的 registry 原語，才能讓後續 consumer 在真 owner/proof 與 action gate 下接線；只改 registry 不會閉合公開 recovery 或母 #497。

## What Changes

僅提案 registry.py 的九組 additive receipt/disposition/revision/checkpoint 契約，完整 Requirements 與母 AC 分工如下；詳細交易設計由同 change 的 design.md 完整承載。此提案不是另一個 work item，也不是正式發布 authority。
