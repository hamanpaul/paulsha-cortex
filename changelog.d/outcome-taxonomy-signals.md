...
- **#826 outcome taxonomy signals**：新增 `effort_not_supported`／`executable_not_found`／`launch_failed`
  ...
  executor 的 shell command-not-found 與可信空 `127` 才會進 `executable_not_found`，缺 log／
  讀不到 log 的 `127` 維持 unknown/hint。`effort_not_supported`／`executable_not_found` 可在既有合格候選內有界 reroute，
